window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // Strict wire codec: the single create() factory face, matching the host
    // half. The client contribution reaches the same DescriptorStore.validate()
    // through ctx.remote.$mount(), so the 0.1.5-line `schema` field is refused
    // here too ("strict codec has no create() factory"). create() returns a
    // shape-defensive parser: the authoritative strict validation of a result
    // is the host's zod schema, which the Remote call runs before it replies.
    ...
    function PromptToggle({ api, t }) {
      ...
      const icon = state.busy
        ? h(IconLoadingOutlineRegular, { size: 14 })
        : state.enabled
